      ...
      <table className="min-w-full bg-white border border-gray-200 rounded-lg">
        <thead>
          <tr className="border-b bg-gray-100">
            <th className="py-2 px-4 text-left text-sm font-medium text-gray-700">Title</th>
            <th className="py-2 px-4 text-left text-sm font-medium text-gray-700">Description</th>
            <th className="py-2 px-4 text-left text-sm font-medium text-gray-700">Due Date</th>
            <th className="py-2 px-4 text-left text-sm font-medium text-gray-700">Completed</th>
        ...
        <tbody>
          {tasks.length === 0 ? (
            <tr>
              <td colSpan="5" className="py-4 text-center text-gray-600">No tasks available</td>
            </tr>
          ) : (
            tasks.map((task) => (
              <tr key={task.id} className="border-b">
                <td className="py-3 px-4 text-sm text-gray-800">{task.title}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{task.description}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{task.due_date}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{task.completed ? "Yes" : "No"}</td>
                <td className="py-3 px-4 flex space-x-2 text-sm">
                  <Link to={`/okr/tasks/edit/${task.id}`} className="text-blue-500 hover:text-blue-600">
